...
class AuthServiceTest {
  ...
  @Mock
  private TokenRepositoryPort tokenRepo;
  ...
  void refreshTokenValidoRetornaNuevoAccessToken() {
    ...
    var tokenGuardado = Token.builder()
      .tokenHash("refresh-hash")
      .user(user)
      .build();
    when(tokenRepo.findByHash("refresh-hash")).thenReturn(
      Optional.of(tokenGuardado)
    );
    when(jwtService.isTokenValid(tokenGuardado)).thenReturn(true);
    when(jwtService.generateAccesToken(user)).thenReturn(
      tokenFalso("nuevo-access")
    );
  ...
  void registroConEmailExistenteLanzaExcepcion() {
    ...
    when(userRepo.findByEmail("carlos@mail.com")).thenReturn(
      Optional.of(new User())
    );

    // Act - Assert
    assertThrows(EmailAlreadyExitsException.class, () ->
      authService.register(request)
    );
  ...
  void loginConEmailInexistenteLanzaExcepcion() {
    ...
    when(userRepo.findByEmail("noexiste@mail.com")).thenReturn(
      Optional.empty()
    );
  ...
  void refreshTokenExpiradoLanzaExcepcion() {
    ...
    var tokenExpirado = Token.builder()
      ...
      .user(new User())
      .build();
    when(tokenRepo.findByHash("expirado")).thenReturn(
      Optional.of(tokenExpirado)
    );
  ...
  void obtenerTokenExpiradoLanzaExcepcion() {
    ...
    var tokenExpirado = Token.builder()
      ...
      .user(user)
      .build();
    when(tokenRepo.findByHash("expirado-hash")).thenReturn(
      Optional.of(tokenExpirado)
    );

    // Act - Assert
    assertThrows(InvalidTokenException.class, () ->
      authService.getToken("expirado-hash")
    );
  }
  ...
  void refreshTokenValidoRotaRefreshToken() {
    // Arrange
    var user = User.builder().userId(UUID.randomUUID()).build();
    var refresh = Token.builder()
      .tokenHash("refresh-original")
      .user(user)
      .build();
    when(tokenRepo.findByHash("refresh-original")).thenReturn(
      Optional.of(refresh)
    );
  ...
  void obtenerTokenRevocadoLanzaExcepcion() {
    ...
    when(tokenRepo.findByHash("revocado-hash")).thenReturn(
      Optional.of(tokenRevocado)
    );

    // Act - Assert
    assertThrows(InvalidTokenException.class, () ->
      authService.getToken("revocado-hash")
    );
  }
}
